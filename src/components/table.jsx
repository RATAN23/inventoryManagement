...
const TableData = () => {
    ...
    const handleVisibility = (val) => {
     const isEditable = editable.some(item => item.name === val.name);
       if(isEditable){
         setEditable(prev => prev.filter((x) => x.name != val.name));
       }else{
          setEditable(prev => [...prev , val]);
       }
    }
                    ...
                    <tr key = {index} className='border-white/20 border-b-2  bg-slate-800 text-white' >
                        ...
                        <td className='px-20 py-2 text-left'>
                            <div className='flex space-x-2'>
                                <EditIcon color={!isAdmin ? "disabled" : "inherit"} onClick= {() => {
                                    isAdmin ? handleEditItem(x) : ''
                                }}/>
                                <VisibilityIcon color={!isAdmin ? "disabled" : "inherit"} onClick = {() =>{
                                    handleVisibility(x);
                                }} />
                                <DeleteIcon color={!isAdmin ? "disabled" : "inherit"}  onClick= {() => {
                                    isAdmin ? handleDeleteItem(x.name) : ''
                                }} />
                            </div>
                        </td>
                    </tr>
